...
#Using XL-430 Motor specification as default, which is the same as XL-330 in control table except for the maximum velocity value
...
class DXL_Motor(object):
    def __init__(self, port_h, package_h, Motor_number =1):
        ...
        self.DXL_MINIMUM_EXTENDED_POSITION_VALUE = -1048575 #Specifically for XL-430 motors and meant to be used in extended position control mode. 
        self.DXL_MAXIMUM_EXTENDED_POSITION_VALUE = 1048575 # 256 revolutions in either direction in extended position control mode.
    ...
    def writeExtendedPosition(self, value):
        if self.OPERATING_MODE == EXTENDED_POSITION_MODE:
            ...
            # self.msg_sent.append((self.DXL_ID, ADDR, LEN, data))
            if value >= self.DXL_MINIMUM_EXTENDED_POSITION_VALUE and value <= self.DXL_MAXIMUM_EXTENDED_POSITION_VALUE:
                self.msg_sent.append((self.DXL_ID, ADDR, LEN, data))
                # _ = DXL_Conmunication.groupBulkWrite.addParam(self.DXL_ID, ADDR, LEN, data)
            else:
                print("Command exceed maximum range")
        else:
            print("Operating Mode Error while setting position")
    ...
    def reversal(self, reversed): #Specifically made for extended motor position and velocity control
        if reversed == True:
            ...
            print("Invalid Request, enter boolean statement")
